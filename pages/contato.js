...
export default function Contato() {
  ...
  async function loadMapScenario() {
    console.log("si");
    const map = await new Microsoft.Maps.Map(
      document.getElementById("map-contato"),
      {}
    );
    setMultViewMap(map);
  }
  ...
  return (
    <Template>
      <Head>
        <script
          src={`http://www.bing.com/api/maps/mapcontrol?callback=loadMapScenario&key=${process.env.mapsKey}`}
          defer
        />
      </Head>
