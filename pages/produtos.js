...
export default function Produtos({ id, type, search }) {
  ...
  function setLogo() {
    ...
    if (pageData.logo) {
      return (
        ...
        >
          <img src={pageData.logo} style={{ width: "120px" }} />
        </div>
      );
  ...
  return (
    <Template>
      {visLoading ? (
        <Row jc="center" ait="center" style={{ width: "100%" }}>
          <img src={loading} style={{ width: "32px" }} />
        </Row>
      ) : (
        <Column style={{ width: "100%" }}>
          <SlideWeb images={images.map((img) => img.image)} />
          <DivLogoFilters
            style={{ width: "100%", margin: "35px 0", flexWrap: "wrap" }}
          >
            {setLogo()}
            <Filters type={type} S />
          </DivLogoFilters>
