  	...
  	async function async() {
          ...
          try{
            ...
            async function loadMapScenario() {
              ...
              map = await new Microsoft.Maps.Map(document.getElementById('map'), {})
              setMap(map)
  ...
  return (
    <Template>
      <Column style={{ width: "100%" }}>
        ...
        <Row style={{ alignItems: "center", maxWidth: "800px",width: "80%", margin: "auto" }} jc="center">
          <Line style={{ marginRight: "25px", minWidth: "50px" }}/>
          <h1 style={{ color: "#5B5B5B", textAlign: "center" }}>Encontre um revendedor próximo a você!</h1>
        ...
        <p style={{ color: "#717171", opacity: .8, textAlign: "center" }}>Selecione a baixo seu estado e cidade para encontrar o revendedor mais próximo.</p>
          <Row jc="center" ait="center" style={{ width: "50%", flexWrap: "wrap"}}>
                  ...
                  >
                          ...
                          {state.states.map((st, i) => <option key={i} value={`${st.id}-${st.estado}`}>{st.estado}</option>)}
                  </Select>
                  ...
                  >
                          ...
                          {state.cities.filter(cit => cit.estadoId == state.state.id ? cit : false ).map((cit, i) => <option key={i} value={cit.id}>{cit.cidade}</option>)}
                  </Select>
                  <button 
        ...
        <Row style={{ flexWrap: "wrap", maxWidth: "70%", margin: "auto" }}>
                {resellers.filter(resel => {
                  console.log("STATE", typeof state, state)
                  if (state) {
                    if((new RegExp(state.state.id).test(resel.address) || new RegExp(state.state.name).test(resel.address)) && new RegExp(state.city).test(resel.address)) {
                      return resel
                    } else {
                      if ((new RegExp(state.state.id).test(resel.address) || new RegExp(state.state.name).test(resel.address))){ 
                        return resel
                      } 
                    }
                  }
