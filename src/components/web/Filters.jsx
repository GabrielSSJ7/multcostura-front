...
export default function Filters ({ type }) {
	const { machinesForFilters, filters: filtersChoosed,machines } = useSelector(state => state.Machine)
	...
	function handleChange(e) {
		if (e.target.value == 0) {
			dispatch(Creators.loadFilters({...filtersSelecteds, [e.target.name]: null}))
			setFiltersSelecteds({...filtersSelecteds, [e.target.name]: null})
			return
		}
		if (e.target.value != 0) {
			dispatch(Creators.loadFilters({...filtersSelecteds, [e.target.name]: e.target.value}))
			setFiltersSelecteds({...filtersSelecteds, [e.target.name]: e.target.value})
		}		
	}
		...
		keys.sort().forEach((k, i) => {
			...
			elements.push(
				<Select 
					...
					style={{ flex: 1, minWidth: "200px", marginRight: '5px', color: filtersSelecteds[k] == null ? "black" : "red" }}
				>
					<option value={0}>{k.replace(/([A-Z])/g, ' $1').trim().toLowerCase()}</option>
					{
					  filters[k].map(f => {
	...
	function renderFilterSelecteds() {
		const keys = Object.keys(filtersSelecteds)
		const elements = []
		keys.forEach((k,i) => filtersSelecteds[k] ? elements.push(<Tag onClick={() => handleDelete(i)}>{filtersSelecteds[k]} <span style={{ color: "grey", marginLeft: "2px" }}>X</span></Tag>) : null)
		if (elements.length > 0)
			elements.push(<Tag onClick={() => handleDelete(null)}>LIMPAR FILTROS</Tag>)
		return elements;
	}
